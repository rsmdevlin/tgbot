...
class _PGConn:
    ...
    def _fix_sql(sql: str) -> str:
        # Конвертируем ? → %s для psycopg2.
        # Экранируем % в строковых литералах → %%, иначе psycopg2 падает с IndexError.
        result = []
        i = 0
        in_str = False
        str_ch = None
        while i < len(sql):
            ...
            if in_str:
                if ch == str_ch:
                    if i + 1 < len(sql) and sql[i+1] == str_ch:
                        result.append(ch); result.append(ch); i += 2; continue
            ...
            if ch == '?':
                result.append('%s')
            elif ch == '%' and i + 1 < len(sql) and sql[i+1] in ('s', '%'):
                result.append('%' + sql[i+1]); i += 2; continue
            else:
                result.append(ch)
            i += 1
        return ''.join(result)
    ...
    def _fix_sql_pg(sql: str) -> str:
        ...
        import re as _r
        s = sql.strip()
        # BIGSERIAL PRIMARY KEY → SERIAL PRIMARY KEY
        s = _r.sub(r'INTEGER\s+PRIMARY\s+KEY\s+AUTOINCREMENT', 'SERIAL PRIMARY KEY', s, flags=_r.IGNORECASE)
        # BIGINT PRIMARY KEY (без AUTOINCREMENT, только одиночный PK) → BIGINT PRIMARY KEY
        # оставим как есть — PostgreSQL принимает BIGINT PRIMARY KEY
        # INSERT OR IGNORE → INSERT ... ON CONFLICT DO NOTHING
        had_or_ignore = bool(_r.search(r'\bINSERT\s+OR\s+IGNORE\b', s, _r.IGNORECASE))
        s = _r.sub(r'\bINSERT\s+OR\s+IGNORE\b', 'INSERT', s, flags=_r.IGNORECASE)
        if had_or_ignore and 'ON CONFLICT' not in s.upper():
            s = s.rstrip().rstrip(';') + ' ON CONFLICT DO NOTHING'
        # INSERT OR REPLACE → INSERT ... ON CONFLICT DO UPDATE (упрощённо — upsert)
    ...
    def execute(self, sql, params=None):
        ...
        if _r2.match(r'\s*(BEGIN|COMMIT|ROLLBACK)\s*', fixed, _r2.IGNORECASE):
            ...
            # BEGIN — psycopg2 с autocommit=False начинает транзакцию автоматически
            class _TxFake:
    ...
    def executescript(self, sql):
        ...
        # BIGSERIAL PRIMARY KEY → SERIAL PRIMARY KEY
        sql = _r3.sub(r'INTEGER\s+PRIMARY\s+KEY\s+AUTOINCREMENT', 'SERIAL PRIMARY KEY', sql, flags=_r3.IGNORECASE)
        # INSERT OR IGNORE → INSERT ... ON CONFLICT DO NOTHING
...
def _safe_migrate():
    ...
    # Каждый ALTER в своём соединении с autocommit=True — иначе PostgreSQL
    # убивает всю транзакцию при первом "column already exists"
    pool = _get_pg_pool()
    for table, column, col_type in migrations:
        ...
        try:
            _mc = pool.getconn()
            _mc.autocommit = True
            _mc.cursor().execute(f"ALTER TABLE {table} ADD COLUMN {column} {col_type}")
        except Exception:
            pass
        finally:
            if _mc:
                try:
                    _mc.autocommit = False
                    pool.putconn(_mc)
                except Exception:
                    pass
...
def do_merge_databases(status_chat_id, status_msg_id):
    ...
    try:
        ...
        # INSERT новых
        conn.execute("""
            INSERT OR IGNORE INTO users (id, username, first_name, last_name, first_seen, last_seen)
        ...
        # UPDATE существующих — один JOIN через temp таблицу, не N подзапросов
        conn.execute("""
            UPDATE users SET
            ...
            FROM _merge_users mu WHERE users.id = mu.id
        """)
    ...
    try:
        ...
        # Чаты — INSERT новых
        conn.execute("""
            INSERT OR IGNORE INTO chats (id, title, chat_type, username, joined_at, last_activity)
        ...
        conn.execute("""
            UPDATE chats SET
                ...
                last_activity = CASE
                    ...
                    THEN chats.last_activity ELSE mc.last_ts
                END
            FROM _merge_chats mc WHERE chats.id = mc.id
        """)
        conn.execute("COMMIT")
    except Exception as e:
        conn.execute("ROLLBACK")
...
if __name__ == "__main__":
    ...
    # Сброс предыдущей сессии (защита от 409 на Railway)
    _poll_time.sleep(3)
    for _att in range(10):
        try:
            ...
            print(f"⚠️  delete_webhook попытка {_att+1}: {_we}")
            _poll_time.sleep(5)
    ...
    while True:
        try:
            ...
            _es = str(_pe)
            if "409" in _es or "Conflict" in _es:
                _poll_restarts += 1
                _w = min(15 * _poll_restarts, 60)
                print(f"⚠️  409 Conflict — жду {_w}с (попытка {_poll_restarts})...")
                _poll_time.sleep(_w)
                try: bot.delete_webhook(drop_pending_updates=True)
                except Exception: pass
            else:
                print(f"❌ Polling упал: {_pe}")
                _poll_time.sleep(5)