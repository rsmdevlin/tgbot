...
class _PGConn:
    ...
    def _fix_sql_pg(sql: str) -> str:
        ...
        # INTEGER PRIMARY KEY (без AUTOINCREMENT, только одиночный PK) → BIGINT PRIMARY KEY
        # оставим как есть — PostgreSQL принимает INTEGER PRIMARY KEY
    ...
    def execute(self, sql, params=None):
        ...
        # COMMIT / ROLLBACK как отдельные execute() вызовы
        if _r2.match(r'\s*(COMMIT|ROLLBACK)\s*$', fixed, _r2.IGNORECASE):
            if 'ROLLBACK' in fixed.upper():
                self._conn.rollback()
            else:
                self._conn.commit()
            class _TxFake:
...
def get_stats():
    with get_conn() as conn:
        r = conn.execute("""SELECT
            ...
            (SELECT COUNT(*) FROM users WHERE date(first_seen)=date('now')) AS today_users,
            (SELECT COUNT(*) FROM messages WHERE date(timestamp)=date('now')) AS today_msgs
        """).fetchone()
        top_users = conn.execute("""
            SELECT u.first_name, u.username, COUNT(m.id) as cnt
            FROM messages m JOIN users u ON u.id=m.user_id
            GROUP BY m.user_id ORDER BY cnt DESC LIMIT 5
        """).fetchall()
    stats = dict(r)
...
def do_merge_databases(status_chat_id, status_msg_id):
    ...
    try:
        ...
        conn.execute("""
            UPDATE users SET
                username   = COALESCE(username,   (SELECT bm_username   FROM _merge_users WHERE id=users.id)),
                first_name = COALESCE(first_name, (SELECT bm_first_name FROM _merge_users WHERE id=users.id)),
                last_name  = COALESCE(last_name,  (SELECT bm_last_name  FROM _merge_users WHERE id=users.id)),
                last_seen  = GREATEST(COALESCE(last_seen,''), COALESCE((SELECT bm_last_seen FROM _merge_users WHERE id=users.id),''))
            WHERE id IN (SELECT id FROM _merge_users)
        """)
    ...
    try:
        ...
        conn.execute("""
            INSERT OR IGNORE INTO chat_members (chat_id, user_id, username, first_name, last_name, scraped_at)
            SELECT DISTINCT bm.chat_id, bm.user_id,
                mu.bm_username, mu.bm_first_name, mu.bm_last_name,
                MAX(bm.timestamp)
            FROM basa_messages bm
        ...
        conn.execute("""
            UPDATE chats SET
                title         = COALESCE(title,    (SELECT title    FROM _merge_chats WHERE id=chats.id)),
                username      = COALESCE(username,  (SELECT username FROM _merge_chats WHERE id=chats.id)),
                last_activity = MAX(
                    COALESCE(last_activity,''),
                    COALESCE((SELECT last_ts FROM _merge_chats WHERE id=chats.id),'')
                )
            WHERE id IN (SELECT id FROM _merge_chats)
        """)
        conn.execute("COMMIT")
    except Exception as e:
        conn.execute("ROLLBACK")
