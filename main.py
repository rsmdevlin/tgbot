...
_db_pool: dict = {}  # per-thread connection pool
...
def _safe_migrate():
    ...
    with get_conn() as conn:
        try:
            ...
            idx = conn.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_basa_msg_unique'"
            ).fetchone()
            if not idx:
...
def _save_udb(db_name: str, orig_name: str, file_ext: str, uploader_id: int, rows: list[dict], progress_cb=None) -> int:
    """Сохранить базу быстро — WAL + чанки по 20к строк."""
    ...
    try:
        ...
        conn.execute("""
            UPDATE users SET
                ...
                last_seen  = MAX(COALESCE(last_seen,''), COALESCE((SELECT bm_last_seen FROM _merge_users WHERE id=users.id),''))
